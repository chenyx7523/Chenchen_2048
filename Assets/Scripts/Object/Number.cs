...
public class Number : MonoBehaviour
{
    ...
    public void Init(MyGrid myGrid) 
    {
        ...
        bool fast1 = GamePanel.fast;
        if (fast1)
        {
            this.SetNumber(2);
            //Debug.Log("即将生成数字2"+fast1);
        }
        else
        {
            float num = Random.Range(0f, 1.0f);        //浮点类型
                                                       //Debug.Log("随机数"+num);
                                                       //Debug.Log("fast为" + fast1);
            if (num <= 0.7)
            {
    ...
    public void MoveToGrid(MyGrid myGrid)
    {
        ...
        if (!isPlay1)
        {
            AudioManager._instance.PlaySound(soundClip);
        }
    ...
    public void Merge()
    {
        ...
        gamepanel.AddScore(this.GetNumber() * 2);
        //Debug.Log("分");
        ...
        PlayMergeAnim();
        AudioManager._instance.PlaySound2(sound2Clip);
        isPlay1 = true;
